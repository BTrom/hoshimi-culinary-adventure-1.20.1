...
public abstract class LightningRodBlockMixin extends RodBlock implements WeatheringCopper, ChangeOverTimeBlock<WeatheringCopper.WeatherState> {
    ...
    @Unique
    private Optional<Block> hoshimimod$getNextBlock() {
        return Optional.of(ModBlocks.EXPOSED_LIGHTNING_ROD.get());
    }

    @Unique
    private Optional<Block> hoshimimod$getWaxedBlock() {
    ...
    public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        ...
        if (random.nextFloat() < f) {
            hoshimimod$tryOxidize(state, level, pos, random);
        }
    }

    @Unique
    private void hoshimimod$tryOxidize(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        // Use simplified oxidation logic that directly uses our blocks
        Optional<Block> nextBlock = hoshimimod$getNextBlock();
    ...
    public Optional<BlockState> getNext(BlockState state) {
        ...
        return hoshimimod$getNextBlock().map(block -> block.withPropertiesOf(state));
    }
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        if (stack.is(Items.HONEYCOMB)) {
            Optional<Block> waxedBlock = hoshimimod$getWaxedBlock();
