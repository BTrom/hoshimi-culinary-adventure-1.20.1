...
import com.botrom.hoshimi_ca_mod.utils.compat.crockpot.cooking.requirement.*;
import com.botrom.hoshimi_ca_mod.utils.compat.crockpot.jei.gui.DrawableNineSliceResource;
...
public abstract class AbstractDrawableRequirement<T extends IRequirement> implements IDrawable {
    ...
    private void drawRequirementBackground(GuiGraphics guiGraphics, int xOffset, int yOffset) {
        var drawable = new DrawableNineSliceResource(Utils.createResourceLocation("textures/gui/jei/requirement_background.png"), 0, 0, 64, 64, this.getWidth(), this.getHeight(), 8, 8, 8, 8, 64, 64);
    ...
    public static List<AbstractDrawableRequirement<? extends IRequirement>> getDrawables(List<IRequirement> requirements) {
        ...
        if (requirements.isEmpty()) {
            builder.add(new AbstractDrawableRequirement<>(null, Component.translatable("integration.hoshimimod.jei.crock_pot_cooking.requirement.no_requirement")) {
                @Override
                public int getWidth() {
