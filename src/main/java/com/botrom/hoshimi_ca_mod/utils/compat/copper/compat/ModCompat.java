package com.botrom.hoshimi_ca_mod.utils.compat.copper.compat;

import com.botrom.hoshimi_ca_mod.HoshimiCulinaryMod;
import com.botrom.hoshimi_ca_mod.utils.compat.copper.IChestHandler;
import com.botrom.hoshimi_ca_mod.utils.compat.copper.IContainerCompat;
import com.botrom.hoshimi_ca_mod.utils.compat.copper.IModCompatModule;
...
public class ModCompat {
    ...
    public static void register(String modId, Supplier<IModCompatModule> moduleSupplier) {
        if (initialized) {
            HoshimiCulinaryMod.LOGGER.warn("ModCompat: Tried to register module for '{}' after initialization!", modId);
            return;
        }
        
        if (MODULES.containsKey(modId)) {
            HoshimiCulinaryMod.LOGGER.warn("ModCompat: Module for '{}' already registered, skipping duplicate", modId);
            return;
        }
        
        // Only create and register if the mod is loaded
        if (/*Services.PLATFORM.isModLoaded(modId)*/ false) {
            try {
                IModCompatModule module = moduleSupplier.get();
                MODULES.put(modId, module);
                HoshimiCulinaryMod.LOGGER.info("ModCompat: Registered compatibility module for '{}'", modId);
            } catch (Exception e) {
                HoshimiCulinaryMod.LOGGER.error("ModCompat: Failed to create module for '{}': {}", modId, e.getMessage());
                e.printStackTrace();
            }
        } else {
            HoshimiCulinaryMod.LOGGER.info("ModCompat: Mod '{}' not loaded, skipping compatibility module", modId);
        }
    }
    ...
    public static void init() {
        if (initialized) {
            HoshimiCulinaryMod.LOGGER.warn("ModCompat: Already initialized!");
            return;
        }

        HoshimiCulinaryMod.LOGGER.info("ModCompat: Initializing {} compatibility modules...", MODULES.size());
        ...
        for (Map.Entry<String, IModCompatModule> entry : MODULES.entrySet()) {
            try {
                ...
                HoshimiCulinaryMod.LOGGER.info("ModCompat: Initialized compatibility for '{}'", entry.getKey());
            } catch (Exception e) {
                HoshimiCulinaryMod.LOGGER.error("ModCompat: Failed to initialize module for '{}': {}", entry.getKey(), e.getMessage());
            }
        }
        
        initialized = true;
        HoshimiCulinaryMod.LOGGER.info("ModCompat: Initialization complete. {} chest handlers, {} container handlers registered.",
