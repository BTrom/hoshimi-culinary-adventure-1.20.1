...
public class EyeBoneRenderer extends BlockEntityWithoutLevelRenderer {

	public EyeBoneRenderer() {
	...
	public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
		if (stack.getItem() instanceof EyeBoneBlockItem chester) {
//			Entity entity = EntityCache.fetchEntity(chester.getChesterEntity(), Minecraft.getInstance().level, stack.getTag() != null && stack.getTag().getBoolean(Chester.EXTENDED_TAG));
//			Entity entity = EntityCache.fetchEntity(chester.getChesterEntity(), Minecraft.getInstance().level);
//			if (entity != null) {
