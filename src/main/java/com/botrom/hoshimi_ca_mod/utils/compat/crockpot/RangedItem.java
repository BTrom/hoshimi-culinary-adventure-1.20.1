...
import com.botrom.hoshimi_ca_mod.utils.compat.crockpot.util.JsonUtils;
...
public class RangedItem {
    ...
    public RangedItem(Item item, int min, int max) {
        ...
        if (min == 0 && max == 0) {
            HoshimiCulinaryMod.loggerWarn("The count of RangedItem is 0, make sure this is intentional!");
        }
        if (min > max) {
            HoshimiCulinaryMod.loggerWarn("The minimum count of RangedItem is greater than the maximum count, make sure this is intentional!");
        }
        this.item = item;
        this.min = min;
        this.max = max;
