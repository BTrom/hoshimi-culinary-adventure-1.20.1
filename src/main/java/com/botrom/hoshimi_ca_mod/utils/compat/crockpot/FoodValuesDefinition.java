...
public class FoodValuesDefinition extends AbstractRecipe<Container> {
    ...
    public static FoodValues getFoodValues(ItemStack stack, Level level) {
        var allDefs = level.getRecipeManager().getRecipesFor(ModRecipes.FOOD_VALUES_RECIPE_TYPE.get(), new SimpleContainer(stack), level);
        if (allDefs.isEmpty()) {
    ...
    public static Set<ItemStack> getMatchedItems(FoodCategory category, Level level) {
        ...
        var allDefs = level.getRecipeManager().getAllRecipesFor(ModRecipes.FOOD_VALUES_RECIPE_TYPE.get()).stream().filter(def -> def.getFoodValues().has(category)).toList();
