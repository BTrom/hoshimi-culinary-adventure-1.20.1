package com.botrom.hoshimi_ca_mod.utils.compat;

import com.botrom.hoshimi_ca_mod.HoshimiCulinaryMod;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.world.entity.Entity;
import net.minecraft.world.entity.LivingEntity;
import net.minecraftforge.common.capabilities.Capability;
import net.minecraftforge.common.capabilities.CapabilityManager;
import net.minecraftforge.common.capabilities.CapabilityToken;
import net.minecraftforge.common.capabilities.RegisterCapabilitiesEvent;
import net.minecraftforge.event.AttachCapabilitiesEvent;
import net.minecraftforge.registries.ForgeRegistries;

public class CapabilityHandler {
	public static final ResourceLocation FRIENDED_CAP = new ResourceLocation(HoshimiCulinaryMod.MOD_ID, "friended");

	public static final Capability<IFriended> CAPABILITY_FRIENDED = CapabilityManager.get(new CapabilityToken<>() {
	});

	public static void register(RegisterCapabilitiesEvent event) {
		event.register(IFriended.class);
	}

	public static void attachCapability(AttachCapabilitiesEvent<Entity> event) {

		if (event.getObject() instanceof LivingEntity livingEntity) {
			ResourceLocation location = ForgeRegistries.ENTITY_TYPES.getKey(livingEntity.getType());
			if (location != null && location.getNamespace().equals(HoshimiCulinaryMod.MOD_ID)) {
				event.addCapability(CapabilityHandler.FRIENDED_CAP, new FriendedCapability());
			}
		}
	}
}
