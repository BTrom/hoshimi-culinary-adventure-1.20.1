...
public class AmbientDesertBlockSoundsPlayer {
    ...
    public static boolean shouldPlayDesertDryVegetationBlockSounds(Level level, BlockPos pos) {
        return level.getBlockState(pos).is(ModTags.TRIGGERS_AMBIENT_DESERT_DRY_VEGETATION_BLOCK_SOUNDS)
            && level.getBlockState(pos.below()).is(ModTags.TRIGGERS_AMBIENT_DESERT_DRY_VEGETATION_BLOCK_SOUNDS);
    }
    ...
    private static boolean canTriggerAmbientDesertSandSounds(BlockState state) {
        return state.is(ModTags.TRIGGERS_AMBIENT_DESERT_SAND_BLOCK_SOUNDS);
    }
}