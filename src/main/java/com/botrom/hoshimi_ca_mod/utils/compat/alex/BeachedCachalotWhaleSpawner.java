...
public class BeachedCachalotWhaleSpawner {
    ...
    public BeachedCachalotWhaleSpawner(ServerLevel p_i50177_1_) {
        ...
        if (this.delay == 0 && this.chance == 0) {
            this.delay = ModConfig.beachedCachalotWhaleSpawnDelay;
    ...
    public void tick() {
        if (ModConfig.beachedCachalotWhales && --this.timer <= 0 && world.isThundering()) {
            this.timer = 1200;
            ...
            if (this.delay <= 0) {
                ...
                if (this.world.getGameRules().getBoolean(GameRules.RULE_DOMOBSPAWNING)) {
                    int i = this.chance;
                    this.chance = Mth.clamp(this.chance + ModConfig.beachedCachalotWhaleSpawnChance, 5, 100);
                    worldinfo.setBeachedCachalotSpawnChance(this.chance);
                    if (this.random.nextInt(100) <= i && this.attemptSpawnWhale()) {
                        this.chance = ModConfig.beachedCachalotWhaleSpawnChance;
                    }
                }
            }
        }
    ...
    private boolean attemptSpawnWhale() {
        ...
        if (playerentity == null) {
            ...
            if (blockpos2 != null && this.func_226559_a_(blockpos2) && blockpos2.distSqr(blockpos) > 225) {
                ...
                CachalotWhaleEntity whale = ModEntities.CACHALOT_WHALE.get().create(world);
    ...
    private BlockPos func_221244_a(BlockPos p_221244_1_, int p_221244_2_) {
        ...
        for(int i = 0; i < 10; ++i) {
            ...
            if (AMWorldRegistry.testBiome(ModConfig.cachalot_whale_beached_spawns, world.getBiome(blockpos1)) && NaturalSpawner.isSpawnPositionOk(Type.ON_GROUND, this.world, blockpos1, EntityType.WANDERING_TRADER)) {
                blockpos = blockpos1;
                break;
            }
        }
