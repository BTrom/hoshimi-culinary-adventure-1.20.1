...
public class DynamicPizzaSliceModel implements IUnbakedGeometry<DynamicPizzaSliceModel>
{
    ...
    public BakedModel bake(IGeometryBakingContext context, ModelBaker baker, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState, ItemOverrides overrides, ResourceLocation modelLocation)
    {
        ...
        ItemStackHandler handler = Utils.createHandlerFromStack(stack, 10);
        ...
        CompositeModel.Baked.Builder builder = CompositeModel.Baked.builder(itemContext, particleSprite, new PizzaSliceOverrideHandler(overrides, baker, itemContext, this), context.getTransforms());
            ...
            for(int j = 0; j < layers.size(); j++)
            {
                ...
                if(sprite != null)
                {
                    ...
                    if(tintIndexes.get(j) != -1)
                    {
                        ...
                        int color = Utils.getDominantColor(Minecraft.getInstance().getItemRenderer().getItemModelShaper().getItemModel(handler.getStackInSlot(tintIndexes.get(j))).getParticleIcon(), false);
