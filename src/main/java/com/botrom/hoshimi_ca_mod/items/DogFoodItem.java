...
public class DogFoodItem extends Item {
    ...
    public @NotNull InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity entity, net.minecraft.world.InteractionHand hand) {
        boolean isWolf = entity.getType().is(ModTags.IS_WOLF);
        if (entity instanceof TamableAnimal wolf && !wolf.isTame() && isWolf) {
            if (!entity.level().isClientSide) {
                wolf.tame(player);
                wolf.setOwnerUUID(player.getUUID());
                wolf.setInSittingPose(false);
                wolf.heal(10.0F);
                wolf.addEffect(new MobEffectInstance(ModEffects.DOG_FOOD.get(), 3600, 0));
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> tooltip, TooltipFlag tooltipFlag) {
        tooltip.add(Component.translatable("tooltip.hoshimimod.animal_fed_to_dog").withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("tooltip.hoshimimod.dog_effect_1").withStyle(ChatFormatting.BLUE));
        tooltip.add(Component.translatable("tooltip.hoshimimod.dog_effect_2").withStyle(ChatFormatting.BLUE));
        tooltip.add(Component.translatable("tooltip.hoshimimod.dog_effect_3").withStyle(ChatFormatting.BLUE));
    }
}
