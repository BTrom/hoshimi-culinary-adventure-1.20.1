...
public class ItemModFishBucket extends MobBucketItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        ...
        if (fishType == ModEntities.LOBSTER.get()) {
...
//        if (fishType == ModEntities.TERRAPIN.get()) {
//            CompoundTag compoundnbt = stack.getTag();
...
//            }
//        }
        if (fishType == ModEntities.COMB_JELLY.get()) {
            CompoundTag compoundnbt = stack.getTag();
            if (compoundnbt != null && compoundnbt.contains("BucketVariantTag", 3)) {
                int i = compoundnbt.getInt("BucketVariantTag");
    ...
    private void addExtraAttributes(Entity entity, ItemStack stack) {
        if(entity instanceof CatfishEntity catfish){
            ...
            }else if(stack.is(ModItems.LARGE_CATFISH_BUCKET.get())){
