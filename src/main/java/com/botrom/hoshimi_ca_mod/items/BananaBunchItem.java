...
public class BananaBunchItem extends Item {
	...
	// TODO: Move into eating a banana instead
	...
	public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
		if (!target.getType().is(ModTags.UNAFFECTED_BY_SLIPPING)) {
	...
	private void placeBanana(Level level, double posX, double posY, double posZ, float yaw) {
		if (!level.isClientSide) {
			BananaPeel bananapeel = ModEntities.BANANA_PEEL.get().create(level);
	...
	private void handleOpening(Level level, Player player, InteractionHand hand, ItemStack stack) {
		if (!level.isClientSide) {
			...
			if (stack.isEmpty()) {
				player.setItemInHand(hand, new ItemStack(ModItems.BANANA.get(), 1 + level.random.nextInt(3)));
			} else {
				ItemStack itemstack = new ItemStack(ModItems.BANANA.get(), 1 + level.random.nextInt(3));
		...
		level.playSound(player, player.blockPosition(), ModSounds.BANANA_BUNCH_OPEN.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
	}
}
