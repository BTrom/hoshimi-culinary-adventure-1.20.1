...
public class FireflyBushBlock extends BushBlock implements SpreadableBonemealableBlock {
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
        if (random.nextInt(FIREFLY_AMBIENT_SOUND_CHANCE_ONE_IN) == 0 && Utils.isMoonVisible(level) && level.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, pos.getX(), pos.getZ()) <= pos.getY()) {
            level.playLocalSound(pos, ModSounds.FIREFLY_BUSH_IDLE.get(), SoundSource.AMBIENT, 1.0F, 1.0F, false);
        }

        if (Utils.isMoonVisible(level) || level.getMaxLocalRawBrightness(pos) <= FIREFLY_SPAWN_MAX_BRIGHTNESS_LEVEL) {
            if (random.nextDouble() <= FIREFLY_CHANCE_PER_TICK) {
                ...
                level.addParticle(ModParticleTypes.FIREFLY.get(), x, y, z, 0.0, 0.0, 0.0);
            }
        }
    }
