...
import com.botrom.hoshimi_ca_mod.entities.ai.HappyGhastAi;
import com.botrom.hoshimi_ca_mod.entities.ai.HappyGhastTemptGoal;
...
import com.botrom.hoshimi_ca_mod.utils.compat.BlockPosUtils;
import com.botrom.hoshimi_ca_mod.utils.compat.CollisionUtils;
import com.botrom.hoshimi_ca_mod.utils.compat.vanillabackport.api.leash.Leashable;
import com.botrom.hoshimi_ca_mod.utils.mixins.vanillabackportsmixins.access.LivingEntityAccessor;
...
public class HappyGhast extends Animal implements PlayerRideable, Leashable {
    public static final Ingredient IS_FOOD = Ingredient.of(ModTags.HAPPY_GHAST_FOOD);
    ...
    private static final Vec3[] PASSENGER_ATTACHMENTS = {
        new Vec3(0.0, 3.75, 1.8),
        new Vec3(-1.8, 3.75, 0.0),
        new Vec3(0.0, 3.75, -1.8),
        new Vec3(1.8, 3.75, 0.0)
    };
        ...
        this.goalSelector
            .addGoal(
                ...
                new HappyGhastTemptGoal(
                    this,
                    1.0,
                    stack -> !this.isHarnessed() && !this.isBaby() ? stack.is(ModTags.HAPPY_GHAST_TEMPT_ITEMS) : IS_FOOD.test(stack),
                    false,
                    7.0
                )
            );
    ...
    public void equipHarness() {
        this.level().playSound(null, this, ModSounds.HARNESS_EQUIP.get(), SoundSource.NEUTRAL, 0.5F, 1.0F);
    }
    ...
    public boolean isHarnessed() {
        return this.getItemBySlot(EquipmentSlot.CHEST).is(ModTags.HARNESSES);
    }
    ...
    protected void positionRider(Entity passenger, MoveFunction callback) {
        if (this.hasPassenger(passenger)) {
            double baseY = this.getY() + passenger.getMyRidingOffset();
            int idx = Math.min(this.getPassengers().indexOf(passenger), PASSENGER_ATTACHMENTS.length - 1);
            Vec3 offset = PASSENGER_ATTACHMENTS[idx].yRot(-this.getYRot() * (float) Math.PI / 180F);

            callback.accept(passenger, this.getX() + offset.x, baseY + offset.y, this.getZ() + offset.z);
        }
    }
    ...
    protected Vec3 getRiddenInput(Player player, Vec3 travelVector) {
        ...
        return new Vec3(forward, upward, strafe).scale(((double) 3.9F * this.getAttributeValue(Attributes.FLYING_SPEED)) * ModConfig.happyGhastSpeedModifier);
    }
    ...
    public boolean vb$supportQuadLeash() {
        return true; // This tells the renderer to look for the 4 corner points
    }
    ...
    static class GhastMoveControl extends MoveControl {
        ...
        private boolean blockTraversalPossible(BlockGetter level, @Nullable Vec3 origin, @Nullable Vec3 target, BlockPos pos, boolean inWater, boolean inLava) {
            ...
            if (state.isAir()) {
                ...
                } else if (state.is(ModTags.HAPPY_GHAST_AVOIDS)) {
                    return false;
                } else {
