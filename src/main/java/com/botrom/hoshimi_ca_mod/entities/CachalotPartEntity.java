...
public class CachalotPartEntity extends PartEntity<CachalotWhaleEntity> {

    private final EntityDimensions size;
    public float scale = 1;

    public CachalotPartEntity(CachalotWhaleEntity parent, float sizeX, float sizeY) {
        super(parent);
        this.size = EntityDimensions.scalable(sizeX, sizeY);
        this.refreshDimensions();
    }

    public CachalotPartEntity(CachalotWhaleEntity cachalotWhaleEntity, float sizeX, float sizeY, EntityDimensions size) {
        super(cachalotWhaleEntity);
        this.size = size;
    }
    ...
    protected void collideWithNearbyEntities() {
        ...
        if (parent != null) {
            entities.stream().filter(entity -> entity != parent && !(entity instanceof CachalotPartEntity && ((CachalotPartEntity) entity).getParent() == parent) && entity.isPushable()).forEach(entity -> entity.push(parent));
        }
    }
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        if(this.level().isClientSide && this.getParent() != null){
            HoshimiCulinaryMod.sendMSGToServer(new MessageInteractMultipart(this.getParent().getId(), hand == InteractionHand.OFF_HAND));
        }
    ...
    public boolean hurt(DamageSource source, float amount) {
        if(this.level().isClientSide && this.getParent() != null && !this.getParent().isInvulnerableTo(source)){
            ...
            if(key != null){
                HoshimiCulinaryMod.sendMSGToServer(new MessageHurtMultipart(this.getId(), this.getParent().getId(), amount, key.toString()));
            }
        }
