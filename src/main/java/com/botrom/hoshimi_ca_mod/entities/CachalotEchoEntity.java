...
public class CachalotEchoEntity extends Entity {
    private static final EntityDataAccessor<Boolean> RETURNING = SynchedEntityData.defineId(CachalotEchoEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> FASTER_ANIM = SynchedEntityData.defineId(CachalotEchoEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> GREEN = SynchedEntityData.defineId(CachalotEchoEntity.class, EntityDataSerializers.BOOLEAN);
    private UUID ownerUUID;
    private int ownerNetworkId;
    private boolean leftOwner;
    private boolean playerLaunched = false;

    public CachalotEchoEntity(EntityType p_i50162_1_, Level p_i50162_2_) {
        super(p_i50162_1_, p_i50162_2_);
    }

    public CachalotEchoEntity(Level worldIn, CachalotWhaleEntity p_i47273_2_) {
        this(ModEntities.CACHALOT_ECHO.get(), worldIn);
        this.setShooter(p_i47273_2_);
    }

    public CachalotEchoEntity(Level worldIn, LivingEntity p_i47273_2_, boolean right, boolean green) {
        this(ModEntities.CACHALOT_ECHO.get(), worldIn);
        this.setShooter(p_i47273_2_);
    ...
    @OnlyIn(Dist.CLIENT)
    public CachalotEchoEntity(Level worldIn, double x, double y, double z, double p_i47274_8_, double p_i47274_10_, double p_i47274_12_) {
        this(ModEntities.CACHALOT_ECHO.get(), worldIn);
        this.setPos(x, y, z);
        this.setDeltaMovement(p_i47274_8_, p_i47274_10_, p_i47274_12_);
    }

    public CachalotEchoEntity(PlayMessages.SpawnEntity spawnEntity, Level world) {
        this(ModEntities.CACHALOT_ECHO.get(), world);
    }
    ...
    public void tick() {
        ...
        if (this.isReturning() && shooter instanceof final CachalotWhaleEntity whale) {
    ...
    protected void onEntityHit(EntityHitResult result) {
        ...
        if (isReturning()) {
            CachalotWhaleEntity whale = null;
            if (entity instanceof CachalotWhaleEntity) {
                whale = (CachalotWhaleEntity) entity;
                if (result.getEntity() instanceof CachalotWhaleEntity || result.getEntity() instanceof CachalotPartEntity) {
                    whale.recieveEcho();
            ...
            if (entity instanceof CachalotWhaleEntity) {
                final Vec3 vec = ((CachalotWhaleEntity) entity).getReturnEchoVector();
                final double d0 = vec.x() - this.getX();
                final double d1 = vec.y() - this.getY();
                final double d2 = vec.z() - this.getZ();
                this.setDeltaMovement(Vec3.ZERO);
                final CachalotEchoEntity echo = new CachalotEchoEntity(this.level(), ((CachalotWhaleEntity) entity));
    ...
    protected boolean canHitEntity(Entity p_230298_1_) {
        ...
        if (this.isReturning()) {
            return p_230298_1_ instanceof CachalotPartEntity || p_230298_1_ instanceof CachalotWhaleEntity;
        } else if (p_230298_1_ instanceof CachalotPartEntity) {
            return false;
        }
