...
public class GiantSquidPartEntity extends PartEntity<GiantSquidEntity> implements IHurtableMultipart {

    private final EntityDimensions size;
    public float scale = 1;
    private boolean collisionOnly = false;

    public GiantSquidPartEntity(GiantSquidEntity parent, float sizeX, float sizeY) {
        super(parent);
        this.size = EntityDimensions.scalable(sizeX, sizeY);
        this.refreshDimensions();
    }

    public GiantSquidPartEntity(GiantSquidEntity parent, float sizeX, float sizeY, boolean collisionOnly) {
    ...
    public boolean hurt(DamageSource source, float amount) {
        if(this.level().isClientSide && this.getParent() != null && !this.getParent().isInvulnerableTo(source) && !collisionOnly){
            HoshimiCulinaryMod.sendMSGToServer(new MessageHurtMultipart(this.getId(), this.getParent().getId(), amount, source.getMsgId()));
        }
