...
import com.botrom.hoshimi_ca_mod.blocks.ChrysalisBlock;
        ...
        public void tick() {
            ...
            if (this.isValidTarget(level, caterpillar.blockPosition())) {
                if (!level.isClientSide) {
                    ((ServerLevel) level).sendParticles(new BlockParticleOption(ParticleTypes.BLOCK, ModBlocks.CHRYSALIS_BLOCK.get().defaultBlockState()), caterpillar.getX(), caterpillar.getY(), caterpillar.getZ(), 50, caterpillar.getBbWidth() / 4.0F, caterpillar.getBbHeight() / 4.0F, caterpillar.getBbWidth() / 4.0F, 0.05D);
                }
                caterpillar.discard();
                level.setBlockAndUpdate(caterpillar.blockPosition(), ModBlocks.CHRYSALIS_BLOCK.get().defaultBlockState().setValue(ChrysalisBlock.FACING, facing));
