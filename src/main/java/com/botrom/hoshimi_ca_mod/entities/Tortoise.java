...
public class Tortoise extends TamableAnimal implements NaturalistGeoEntity, HidingAnimal, EggLayingAnimal {
    ...
    private static final Ingredient TEMPT_ITEMS = Ingredient.of(ModTags.TORTOISE_TEMPT_ITEMS);
    ...
    private void soundListener(SoundKeyframeEvent<Tortoise> event) {
        ...
        if (animatable.level().isClientSide) {
            if (event.getKeyframeData().getSound().equals("hide")) {
                animatable.level().playLocalSound(animatable.getX(), animatable.getY(), animatable.getZ(), ModSounds.TORTOISE_HIDE.get(), animatable.getSoundSource(), 0.5F, 1.0F, false);
            }
            if (event.getKeyframeData().getSound().equals("thud")) {
                animatable.level().playLocalSound(animatable.getX(), animatable.getY(), animatable.getZ(), ModSounds.TORTOISE_THUD.get(), animatable.getSoundSource(), 0.5F, 1.0F, false);
            }
        }
    }
