...
import com.botrom.hoshimi_ca_mod.entities.ai.AnimalAIRandomSwimming;
import com.botrom.hoshimi_ca_mod.entities.ai.AquaticMoveController;
import com.botrom.hoshimi_ca_mod.entities.ai.SwimmerJumpPathNavigator;
...
public class FlyingFishEntity extends WaterAnimal implements FlyingAnimal, Bucketable {

    private static final EntityDataAccessor<Boolean> GLIDING = SynchedEntityData.defineId(FlyingFishEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(FlyingFishEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> FROM_BUCKET = SynchedEntityData.defineId(FlyingFishEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public FlyingFishEntity(EntityType<? extends WaterAnimal> type, Level level) {
        super(type, level);
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, MobSpawnType spawnReasonIn) {
        return ModEntities.rollSpawn(ModConfig.flyingFishSpawnRolls, this.getRandom(), spawnReasonIn);
    }
    ...
    public boolean hurt(DamageSource source, float amount) {
        ...
        if(prev && source.getEntity() != null){
            double range = 15;
            this.glideIn = 0;
            List<? extends FlyingFishEntity> list = this.level().getEntitiesOfClass(this.getClass(), this.getBoundingBox().inflate(range, range/2, range));
            for(FlyingFishEntity fsh : list){
                fsh.glideIn = 0;
            }
        }
        return prev;
    ...
    public ItemStack getBucketItemStack() {
        ItemStack stack = new ItemStack(ModItems.FLYING_FISH_BUCKET.get());
    ...
    private class GlideGoal extends Goal {
        ...
        public GlideGoal(FlyingFishEntity fish) {
