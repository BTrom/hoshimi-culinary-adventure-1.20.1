...
public class MimicOctopusRenderer extends MobRenderer<MimicOctopusEntity, MimicOctopusModel> {
    private static final ResourceLocation TEXTURE = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus.png");
    private static final ResourceLocation TEXTURE_OVERLAY = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus_overlay.png");
    private static final ResourceLocation TEXTURE_CREEPER = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus_creeper.png");
    private static final ResourceLocation TEXTURE_GUARDIAN = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus_guardian.png");
    private static final ResourceLocation TEXTURE_PUFFERFISH = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus_pufferfish.png");
    private static final ResourceLocation TEXTURE_MIMICUBE = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus_mimicube.png");
    private static final ResourceLocation TEXTURE_EYES = Utils.createResourceLocation("textures/entity/mimic_octopus/mimic_octopus_eyes.png");
    private static final ResourceLocation GUARDIAN_BEAM_TEXTURE = new ResourceLocation("textures/entity/mimic_octopus/guardian_beam.png");
    private static final RenderType BEAM_RENDER_TYPE = RenderType.entityCutoutNoCull(GUARDIAN_BEAM_TEXTURE);

    public MimicOctopusRenderer(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn, new MimicOctopusModel(), 0.4F);
    ...
    public void render(MimicOctopusEntity entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        if (livingentity != null) {
            ...
            float f13 = Mth.cos(f7 + AMMaths.QUARTER_PI) * 0.282F;
            float f14 = Mth.sin(f7 + AMMaths.QUARTER_PI) * 0.282F;
    ...
    protected void scale(MimicOctopusEntity octo, PoseStack matrixStackIn, float partialTickTime) {
        matrixStackIn.translate(0, -0.02F, 0);
        matrixStackIn.scale(0.9F * octo.getScale(), 0.9F * octo.getScale(), 0.9F * octo.getScale());
    }

    public boolean shouldRender(MimicOctopusEntity livingEntityIn, Frustum camera, double camX, double camY, double camZ) {
    ...
    public ResourceLocation getTextureLocation(MimicOctopusEntity entity) {
        return TEXTURE;
    }

    static class OverlayLayer extends RenderLayer<MimicOctopusEntity, MimicOctopusModel> {


        public OverlayLayer(MimicOctopusRenderer render) {
            super(render);
        }

        public void render(PoseStack matrixStackIn, MultiBufferSource buffer, int packedLightIn, MimicOctopusEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            ...
            float finB = 1.0F;
            float finA = 1.0F;
            if (entitylivingbaseIn.getPrevMimicState() == MimicOctopusEntity.MimicState.OVERLAY) {
            ...
            if ((entitylivingbaseIn.getMimicState() == MimicOctopusEntity.MimicState.OVERLAY)) {
            ...
            if (entitylivingbaseIn.getPrevMimicState() != null) {
                ...
                VertexConsumer prev = buffer.getBuffer(ModRenderTypes.entityTranslucent(getFor(entitylivingbaseIn.getPrevMimicState())));
            ...
            VertexConsumer current = buffer.getBuffer(ModRenderTypes.entityTranslucent(getFor(entitylivingbaseIn.getMimicState())));
            ...
            VertexConsumer eyes = buffer.getBuffer(ModRenderTypes.entityTranslucent(TEXTURE_EYES));
        ...
        public ResourceLocation getFor(MimicOctopusEntity.MimicState state) {
            ...
            if (state == MimicOctopusEntity.MimicState.MIMICUBE) {
