...
public class BananaPeel extends Entity {
	...
	public BananaPeel(PlayMessages.SpawnEntity spawnEntity, Level world) {
		this(ModEntities.BANANA_PEEL.get(), world);
	}
	...
	public void push(Entity entityIn) {
		super.push(entityIn);
		if (this.onGround() && !this.isInWater() && entityIn instanceof LivingEntity && this.getY() <= entityIn.getY() && !entityIn.getType().is(ModTags.UNAFFECTED_BY_SLIPPING) && !this.level().isClientSide()) {
			((LivingEntity) entityIn).addEffect(new MobEffectInstance(ModEffects.SLIPPING.get(), 100));
		}
	}
