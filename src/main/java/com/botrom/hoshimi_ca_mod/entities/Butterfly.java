...
import com.botrom.hoshimi_ca_mod.entities.ai.FlyingWanderGoal;
...
//    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData spawnData, @Nullable CompoundTag dataTag) {
        if (reason == MobSpawnType.BUCKET) {
            ...
            this.setVariant(((ButterflyGroupData)spawnData).getVariant(randomSource));
    ...
    public static boolean checkButterflySpawnRules(EntityType<? extends Butterfly> pType, ServerLevelAccessor pLevel, MobSpawnType pReason, @NotNull BlockPos pPos, RandomSource pRandom) {
        return pLevel.getBlockState(pPos.below()).is(ModTags.BUTTERFLIES_SPAWNABLE_ON);
    }
...
//    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return ModEntities.CATERPILLAR.get().create(level);
    }
    ...
    public enum Variant {
        ...
        public static @NotNull Variant getCommonSpawnVariant(RandomSource random) {
            return getSpawnVariant(random, true);
        }

        private static @NotNull Variant getSpawnVariant(RandomSource random, boolean common) {
            Variant[] variants = Arrays.stream(BY_ID).filter((variant) -> {
