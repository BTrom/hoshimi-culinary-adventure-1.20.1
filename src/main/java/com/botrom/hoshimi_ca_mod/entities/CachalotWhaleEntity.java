...
public class CachalotWhaleEntity extends Animal {
    ...
    private static final EntityDataAccessor<Boolean> CHARGING = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> SLEEPING = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> BEACHED = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> ALBINO = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DESPAWN_BEACH = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> GRABBING = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> HOLDING_SQUID_LEFT = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> CAUGHT_ID = SynchedEntityData.defineId(CachalotWhaleEntity.class, EntityDataSerializers.INT);
    public final double[][] ringBuffer = new double[64][3];
    public final CachalotPartEntity headPart;
    public final CachalotPartEntity bodyFrontPart;
    public final CachalotPartEntity bodyPart;
    public final CachalotPartEntity tail1Part;
    public final CachalotPartEntity tail2Part;
    public final CachalotPartEntity tail3Part;
    public final CachalotPartEntity[] whaleParts;
    ...
    public CachalotWhaleEntity(EntityType type, Level world) {
        super(type, world);
        this.setPathfindingMalus(BlockPathTypes.WATER, 0.0F);
        this.moveControl = new AnimalSwimMoveControllerSink(this, 1, 1, 6);
        this.lookControl = new SmoothSwimmingLookControl(this, 4);
        this.headPart = new CachalotPartEntity(this, 3.0F, 3.5F);
        this.bodyFrontPart = new CachalotPartEntity(this, 4.0F, 4.0F);
        this.bodyPart = new CachalotPartEntity(this, 5.0F, 4.0F);
        this.tail1Part = new CachalotPartEntity(this, 4.0F, 3.0F);
        this.tail2Part = new CachalotPartEntity(this, 3.0F, 2.0F);
        this.tail3Part = new CachalotPartEntity(this, 3.0F, 0.7F);
        this.whaleParts = new CachalotPartEntity[]{this.headPart, this.bodyFrontPart, this.bodyPart, this.tail1Part, this.tail2Part, this.tail3Part};
    }
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.CACHALOT_WHALE_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.CACHALOT_WHALE_HURT.get();
    }
    ...
    public void scaleParts() {
        for (CachalotPartEntity parts : whaleParts) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new AnimalAIRandomSwimming(this, 0.6D, 10, 24, true) {
            public boolean canUse() {
                return !CachalotWhaleEntity.this.isSleeping() && !CachalotWhaleEntity.this.isBeached() && super.canUse();
            }
        });
        ...
        this.targetSelector.addGoal(2, new EntityAINearestTarget3D(this, LivingEntity.class, 30, false, true, ModEntities.buildPredicateFromTag(ModTags.CACHALOT_WHALE_TARGETS)) {
            public boolean canUse() {
                return !CachalotWhaleEntity.this.isSleeping() && !CachalotWhaleEntity.this.isBeached() && super.canUse();
            }
        });
    ...
    public void breakBlock() {
        ...
        if (!this.level().isClientSide && this.blockBreakCounter == 0 && net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(level(), this)) {
            final TagKey<Block> breakables = this.isCharging() && this.getTarget() != null && ModConfig.cachalotDestruction ? ModTags.CACHALOT_WHALE_BREAKABLES : ModTags.ORCA_BREAKABLES;
    ...
    private void spawnSpoutParticles() {
        if (this.isAlive()) {
            ...
            for (int j = 0; j < 5 + random.nextInt(4); ++j) {
                final float angle = (AMMaths.STARTING_ANGLE * this.yBodyRot);
                ...
                this.level().addParticle(ModParticleTypes.WHALE_SPLASH.get(), this.headPart.getX() + extraX, this.headPart.getY() + this.headPart.getBbHeight(), this.headPart.getZ() + extraZ, motX * 0.1F + this.getDeltaMovement().x, 2F, motZ * 0.1F + this.getDeltaMovement().z);
            }
        }
    }
    ...
    protected float getSoundVolume() {
        return this.isSilent() ? 0 : (float) ModConfig.cachalotVolume;
    }
    ...
    public void aiStep() {
        ...
        if (rewardPlayer != null && !hasRewardedPlayer && this.isInWaterOrBubble()) {
            ...
            if (this.distanceTo(rewardPlayer) < 10F) {
                if (!this.level().isClientSide) {
                    ...
                    final ItemEntity itementity = new ItemEntity(this.level(), vec.x, vec.y, vec.z, new ItemStack(ModItems.AMBERGRIS.get(), 2 + random.nextInt(2)));
        ...
        if (!this.isNoAi()) {
            ...
            for (int k = 0; k < 3; ++k) {
                final CachalotPartEntity enderdragonpartentity;
                if (k == 0) {
        ...
        if (!this.level().isClientSide) {
            ...
            if (target == null || !target.isAlive()) {
                ...
                if (isGrabbing() && this.getTarget().isAlive()) {
                    ...
                    if (this.getTarget() instanceof GiantSquidEntity) {
                        if (((GiantSquidEntity) this.getTarget()).tickCaptured(this)) {
                            this.setGrabbing(false);
                    ...
                    if (waitForEchoFlag && !receivedEcho) {
                        ...
                        if (echoTimer % 10 == 0) {
                            if (echoTimer % 40 == 0) {
                                this.playSound(ModSounds.CACHALOT_WHALE_CLICK.get(), this.getSoundVolume(), this.getVoicePitch());
                                this.gameEvent(GameEvent.ENTITY_ROAR);
                            }
                            final CachalotEchoEntity echo = new CachalotEchoEntity(this.level(), this);
                            final float radius = this.headPart.getBbWidth() * 0.5F;
                            final float angle = (AMMaths.STARTING_ANGLE * this.yBodyRot);
                            final double extraX = (radius * (1F + random.nextFloat() * 0.13F)) * Mth.sin(Mth.PI + angle) + (random.nextFloat() - 0.5F) + this.getDeltaMovement().x * 2F;
                            final double extraZ = (radius * (1F + random.nextFloat() * 0.13F)) * Mth.cos(angle) + (random.nextFloat() - 0.5F) + this.getDeltaMovement().z * 2F;
                        ...
                        if (chargeCooldown <= 0 && Math.abs(Mth.wrapDegrees(targetYaw) - Mth.wrapDegrees(this.getYRot())) < 4) {
                            ...
                            if (this.isCharging()) {
                                if (this.distanceTo(target) < this.getBbWidth() && chargeProgress > 4) {
                                    if (target instanceof GiantSquidEntity && !this.isBaby()) {
                                    ...
                                    if (random.nextInt(10) == 0) {
                                        ...
                                        ItemEntity itementity = new ItemEntity(this.level(), vec.x, vec.y, vec.z, new ItemStack(ModItems.CACHALOT_WHALE_TOOTH.get()));
                                        itementity.setDefaultPickUpDelay();
                                        level().addFreshEntity(itementity);
                                    }
                                }
        ...
        if (this.isAlive() && isCharging()) {
            for (final Entity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.headPart.getBoundingBox().inflate(1.0D))) {
                if (!isAlliedTo(entity) && !(entity instanceof CachalotPartEntity) && entity != this) {
                    launch(entity, true);
                }
            }
        }
    ...
    private void launch(Entity e, boolean huge) {
        if ((e.onGround() || e.isInWater()) && !(e instanceof CachalotWhaleEntity)) {
    ...
    private void setPartPosition(CachalotPartEntity part, double offsetX, double offsetY, double offsetZ) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
        CachalotWhaleEntity whale = ModEntities.CACHALOT_WHALE.get().create(serverWorld);
        whale.setAlbino(this.isAlbino());
        return whale;
    }

    public boolean attackEntityPartFrom(CachalotPartEntity cachalotPartEntity, DamageSource source, float amount) {
    ...
    public boolean checkSpawnRules(LevelAccessor worldIn, MobSpawnType spawnReasonIn) {
        return ModEntities.rollSpawn(ModConfig.cachalotWhaleSpawnRolls, this.getRandom(), spawnReasonIn);
    }
    ...
    class AIBreathe extends Goal {
        ...
        public boolean canUse() {
            return CachalotWhaleEntity.this.getAirSupply() < 140;
        }
        ...
        private void navigate() {
            final Iterable<BlockPos> lvt_1_1_ = BlockPos.betweenClosed(Mth.floor(CachalotWhaleEntity.this.getX() - 1.0D), Mth.floor(CachalotWhaleEntity.this.getY()), Mth.floor(CachalotWhaleEntity.this.getZ() - 1.0D), Mth.floor(CachalotWhaleEntity.this.getX() + 1.0D), Mth.floor(CachalotWhaleEntity.this.getY() + 8.0D), Mth.floor(CachalotWhaleEntity.this.getZ() + 1.0D));
            BlockPos lvt_2_1_ = null;

            for (final BlockPos lvt_4_1_ : lvt_1_1_) {
                if (this.canBreatheAt(CachalotWhaleEntity.this.level(), lvt_4_1_)) {
                    lvt_2_1_ = lvt_4_1_.below((int) (CachalotWhaleEntity.this.getBbHeight() * 0.25d));
                    break;
                }
            }

            if (lvt_2_1_ == null) {
                lvt_2_1_ = AMBlockPos.fromCoords(CachalotWhaleEntity.this.getX(), CachalotWhaleEntity.this.getY() + 4.0D, CachalotWhaleEntity.this.getZ());
            }
            if (CachalotWhaleEntity.this.isEyeInFluid(FluidTags.WATER)) {
                CachalotWhaleEntity.this.setDeltaMovement(CachalotWhaleEntity.this.getDeltaMovement().add(0, 0.05F, 0));
            }

            CachalotWhaleEntity.this.getNavigation().moveTo(lvt_2_1_.getX(), lvt_2_1_.getY(), lvt_2_1_.getZ(), 0.7D);
        }
