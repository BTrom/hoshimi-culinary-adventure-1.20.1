...
//              return blockColors.getColor(BlockState, null, null, tintIndex); }, ModBlocks.OLIVE_LEAVES.get(), ModBlocks.FRUIT_OLIVE_LEAVES.get());
        ...
        blockColors.register((state, world, pos, tintIndex) ->
        {
            ...
            int color = Utils.getDominantColor(Minecraft.getInstance().getItemRenderer().getItemModelShaper().getItemModel(handler.getStackInSlot(tintIndex)).getParticleIcon(), isRaw);
        ...
        itemColors.register((stack, tintIndex) ->
        {
            ItemStackHandler handler = Utils.createHandlerFromStack(stack, 12);
            int color = Utils.getDominantColor(Minecraft.getInstance().getItemRenderer().getItemModelShaper().getItemModel(handler.getStackInSlot(tintIndex)).getParticleIcon(), stack.getItem() == ModBlocks.RAW_PIZZA.get().asItem());
            if(handler.getStackInSlot(tintIndex).getItem() instanceof PotionItem) color = PotionUtils.getColor(handler.getStackInSlot(tintIndex));
            if(handler.getStackInSlot(tintIndex).isEmpty()) return 14858625;
            return color; }, ModBlocks.RAW_PIZZA.get(), ModBlocks.PIZZA.get());
    }
