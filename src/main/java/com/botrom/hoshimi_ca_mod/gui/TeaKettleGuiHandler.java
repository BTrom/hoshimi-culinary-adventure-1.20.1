...
public class TeaKettleGuiHandler extends AbstractContainerMenu {
    ...
    public TeaKettleGuiHandler(int syncId, Inventory playerInventory, Container container, ContainerData propertyDelegate) {
        super(ModMenuTypes.TEA_KETTLE_SCREEN_HANDLER.get(), syncId);
        ...
        addSlot(new Slot(container, 6, 118, 43) {
            ...
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(ModTags.SMALL_WATER_FILL) || stack.is(ModTags.LARGE_WATER_FILL);
            }
        });
        addSlot(new Slot(container, 7, 95, 58) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(ModTags.HEAT_ITEMS);
            }
        });
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
    ...
    public @NotNull ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < 8) {
            ...
            if (index < this.slots.size()) {
                if (item.is(ModTags.CONTAINER_ITEMS)) {
                    if (!this.moveItemStackTo(item, 5, 6, false)) {
                        return ItemStack.EMPTY;
                    }
                } else if (item.is(ModTags.HEAT_ITEMS)) {
                    if (!this.moveItemStackTo(item, 7, 8, false)) {
                        return ItemStack.EMPTY;
                    }
                } else if (item.is(ModTags.SMALL_WATER_FILL) || item.is(ModTags.LARGE_WATER_FILL)) {
